...
import {BsFillBookmarkStarFill,BsQuestionCircle,BsTwitter,BsInstagram} from 'react-icons/bs'
  ...
  return <Box display="block"  overflow="clip">
             <Flex alignItems='center'>
                ...
                <Popover placement='top-start' >
                    <PopoverTrigger >
                    <Button variant='ghost'><Text fontSize={'20px'} color='gray.700'>{head}</Text></Button>
                    </PopoverTrigger>
                ...
                <TabPanels margin={"20px 20px 0 20px"}>
                    <TabPanel h="530px">
                        ...
                        <Box w="80%" h="530px" overflowY="scroll" padding="15px" >
                            <Box padding={'15px'} overflowY='scroll'  marginBottom='30px' borderRadius={'10px'} height='auto' boxShadow={'rgba(0, 0, 0, 0.24) 0px 3px 8px'}>
                            ...
                            <Box  borderRadius={'10px'} height='auto' boxShadow={'rgba(0, 0, 0, 0.24) 0px 3px 8px'} >
                                ...
                                <Flex direction='column'>
                                {
                                    from_today.map((el)=>
                                    {
                                        return <Box w='100%' padding='15px' h='120px' maxHeight='auto' key={el.id}>
                                        <Flex padding='0 10px' direction='row'  gap='15px' justifyContent='space-between' w='100%' alignItems='center'>
                                            <Box><Text color='gray.600' fontWeight='bold'>{el.name.toUpperCase()}</Text></Box>
                                            <Divider color='gray.700' />
                    ...
                    <TabPanel>
                        <Flex  h='auto' direction='column' overflowY='scroll'>
                            <Flex alignItems='center' h='40px'justifyContent={'space-between'} width='100%' padding='10px'>
                                <Text fontSize={'25px'} fontWeight='semibold' color='gray.600'>Social Analytics</Text>
                                <Text fontSize={'13px'} gap='10px' alignItems='center' fontWeight='semibold' color='gray.600' display={'flex'}><BsQuestionCircle/>Get help with analytics</Text>
                            </Flex>
                            <Grid padding='30px 10px' templateColumns='repeat(4, 1fr)' templateRows="auto" gap={6} w='82%' overflowY='scroll' className='analytics' >
                                {
                                    AnData.map((el)=>
                                    {
                                        return<Box  w='230px' h='210px' borderRadius='8px' key={el.id} boxShadow="rgba(60, 64, 67, 0.3) 0px 1px 2px 0px, rgba(60, 64, 67, 0.15) 0px 2px 6px 2px">
                                        <Box h='60px' borderRadius='8px 8px 0 0' bg={el.color} display='flex' justifyContent='center' alignItems='center'>{el.icon}</Box>
                                        <Flex direction='column' padding='10px 20px' w='100%' textAlign='left' gap='5px' >
                                            <Text fontWeight='semibold' color='gray.600'>{el.media}</Text>
                                            <Text fontSize='12px' color='gray.600'>{el.text}</Text>
                                        </Flex>
                                    </Box>
                                    })
                                }
